...
st.subheader("Billable Hours By Associate")
billableHoursByAssociate = df[(df['Billable Hours'] != 0)].groupby(['Transaction Calendar Month','Region Supported', 'Colleague  Name'])\
['Billable Hours'].sum()
...
st.subheader("Billable Equivalent Hours By Associate")
billableEquivalentHoursByAssociate = df[(df['Billable Equivalent Hours'] != 0) & (df['Customer Name'] != 'Unspecified')].groupby(['Transaction Calendar Month','Region Supported', 'Colleague  Name','Task Number - Name'])\
...
st.subheader("Non-Billable Hours By Associate")
nonBillableHoursByAssociate = df[(df['Non Billable Hours*'] != 0) & (df['Task Number - Name'] != '01.01-Internal Management')].groupby(['Transaction Calendar Month','Region Supported','Colleague  Name','Task Number - Name'])['Non Billable Hours*'].sum()
